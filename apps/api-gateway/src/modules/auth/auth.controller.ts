...
export class AuthController {
    ...
    @ApiResponse({ status: 200, description: 'Device fingerprint verified successfully' })
    @ApiResponse({ status: 400, description: 'Invalid fingerprint data' })
    @Post('login/fingerprint/email-verification')
    @Public()
    @HttpCode(HttpStatus.OK)
    async verifyFingerprint(@Body() fingerprintDto: FingerprintEmailVerificationDto): Promise<Response> {
    ...
    @Public()
    @HttpCode(HttpStatus.OK)
    async logout(@Body() logoutDto: LogoutDto, @Headers('authorization') authorization: string): Promise<Response> {
    ...
    @HttpCode(HttpStatus.OK)
    async getCurrentUser(@CurrentUser() user: AuthenticatedUser, @Headers('authorization') authorization: string): Promise<Response> {
        return {
            success: true,
            statusCode: 200,
    ...
    @UseGuards(AuthGuard)
    @HttpCode(HttpStatus.OK)
    async getActiveSessions(@Headers('authorization') authorization: string): Promise<Response> {
    ...
    @UseGuards(AuthGuard)
    @HttpCode(HttpStatus.OK)
    async revokeAllSessions(@Headers('authorization') authorization: string): Promise<Response> {
        return this.authService.revokeAllSessions(authorization);
    }

    @ApiOperation({ summary: 'Validate access token' })
    @ApiResponse({ status: 200, description: 'Access token validated successfully' })
    @ApiResponse({ status: 401, description: 'Invalid access token' })
    @Post('session/validate-access')
    @Public()
    @HttpCode(HttpStatus.OK)
    async validateAccess(@Body() validateAccessDto: ValidateAccessDto, @Headers('authorization') authorization: string): Promise<Response> {
        return this.authService.validateAccess(validateAccessDto.access_token, authorization);
    }
    ...
    @UseGuards(AuthGuard)
    @HttpCode(HttpStatus.OK)
    async createSsoToken(@CurrentUser() user: AuthenticatedUser, @Headers('authorization') authorization: string): Promise<Response> {
    ...
    @Public()
    @HttpCode(HttpStatus.OK)
    async validateSsoToken(@Body() validateSsoTokenDto: ValidateSsoTokenDto, @Headers('authorization') authorization: string): Promise<Response> {
        return this.authService.validateSsoToken(validateSsoTokenDto.sso_token, authorization);
    }
    ...
    @UseGuards(AuthGuard)
    @HttpCode(HttpStatus.OK)
    async verifyEmailForgotPassword(@Body() verifyEmailDto: VerifyEmailChangePasswordDto, @Headers('authorization') authorization: string): Promise<Response> {
        return this.authService.verifyEmailForgotPassword(verifyEmailDto.code, authorization);
    }

    @ApiOperation({ summary: 'Change forgot password' })
    @ApiResponse({ status: 200, description: 'Password changed successfully' })
    @ApiResponse({ status: 400, description: 'Invalid password data or verification code' })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    @ApiBearerAuth('JWT-auth')
    @Post('password/forgot/change')
    @UseGuards(AuthGuard)
    @HttpCode(HttpStatus.OK)
    async changeForgotPassword(@Body() changeForgotPasswordDto: ChangeForgotPasswordDto, @Headers('authorization') authorization: string): Promise<Response> {
        return this.authService.changeForgotPassword(changeForgotPasswordDto.code, changeForgotPasswordDto.new_password, authorization);
    }
}
