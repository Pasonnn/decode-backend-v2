...
export abstract class BaseHttpClient {
    ...
    protected async post<T>(url: string, data?: any, config?: AxiosRequestConfig): Promise<Response<T>> {
        try {
            const response: AxiosResponse<Response<T>> = await firstValueFrom(
                this.httpService.post<Response<T>>(`${this.baseURL}${url}`, data, config)
            );
            return response.data;
        } catch (error) {
    ...
    protected async put<T>(url: string, data?: any, config?: AxiosRequestConfig): Promise<Response<T>> {
        try {
            const response: AxiosResponse<Response<T>> = await firstValueFrom(
                this.httpService.put<Response<T>>(`${this.baseURL}${url}`, data, config)
            );
            return response.data;
        } catch (error) {
    ...
    private handleError(error: any, method: string, url: string): never {
        this.logger.error(`HTTP ${method} ${this.baseURL}${url} failed: ${error.message}`);
